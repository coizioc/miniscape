...
def get_gather_list():
    ...
    for level, itemid in sorted(gatherables):
        out += f'**{items.get_attr(itemid).title()}** *(level: {level}, ' \
               f'xp: {items.get_attr(itemid, key=items.XP_KEY)})*\n'
        if len(out) > 1800:
            messages.append(out)
            out = GATHER_HEADER
    messages.append(out)
...
def get_runecraft(person, *args):
    ...
    xp = int(XP_FACTOR * int(number) * items.get_attr(itemid, key=items.XP_KEY))
    users.update_inventory(person.id, loot)
